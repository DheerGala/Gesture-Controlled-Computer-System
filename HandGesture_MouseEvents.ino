...
void loop()
{
  find_distance(); // this function will stores the current distance measured by the ultrasonic sensor in the global variable "distanceL and distanceR"
                   // no matter what, the program has to call this "find_distance" function continuously to get the distance value at all time.
  if(distanceL>15 && distanceR>15 && distanceL<35 && distanceR<35)    //detect both hands
   {
   ...
   if(distanceL<=35 && distanceL>=15)
   {
    temp=distanceL;                         // store the current position of our hand in the variable temp.
    while(distanceL<=50 || distanceL==0)    // this loop will run untill we removes our hand in front of the left sensor.
    {
      find_distance();                      // call this function continuously to get the live data.
      if((temp+6)<distanceL)                // this condition becomes true if we moves our hand away from the left sensor (**but in front of it ). here " temp+6 " is for calibration.
      {
        Serial.println("forward");          // send "forward" serially
        delay (500);
      }
      else if((temp-6)>distanceL)           // this condition becomes true if we moves our hand closer to the left sensor.
      {
        Serial.println("rewind");           // send "rewing" serially
        delay (500);
      }
    }
   }
   
  if(distanceR<=35 && distanceR>=15) // once if we placed our hands in front of the right sensor in the range between 15 to 35cm this condition becomes true.
  { 
    ...
    find_distance();
    if(distanceR<=35 && distanceR>=15) // this condition will true if we place our hand in front of the right sensor for more then 300 milli seconds. 
    {
     temp=distanceR;                         // store the current position of our hand in the variable temp. 
     while(distanceR<=50 || distanceR==0)    // this loop will run untill we removes our hand in front of the right sensor.
     {
       find_distance();                      // call this function continuously to get the live data. 
       if((temp+6)<distanceR)                // this condition becomes true if we moves our hand away from the right sensor (**but in front of it ). here " temp+6 " is for calibration.
       {
       Serial.println("down");               // send "down" serially.
       delay (500);
       }
       else if((temp-6)>distanceR)           // this condition becomes true if we moves our hand closer to the right sensor.
       {
  ...
  else if(distanceL<=35 && distanceL>=15)   // once if we placed our hands in front of the left sensor in the range between 15 to 35cm this condition becomes true.
  { 
    temp=millis();                           
    ...
    {
       find_distance();
       if(distanceR<=35 && distanceR>=15)  // if our hand detects in the right sensor before 300 milli seconds this condition becomes true. ( usually it happens if we swipe our hand from left to right sensor )
       {
    ...
    while(distanceL<=35 && distanceL>=15) // this loop will rotate untill we removes our hand infront of the left sensor. this will avoid not to enter this if condition again.
    find_distance();    
    delay (500);                  
    }
